...
prompts = {
    ...
    'displaying_sources_prompt': "\n\nTo provide additional context for your {type}, here are some sources. If you use information from a source to write a Note, remember to cite the exact URL. Here are URLs and summaries of the sources written by you. {sources}",
    'source_prompt': "\n\nURL: {url}\nSummary: {summary}",
    'relevant_source_prompt': "\n\nNow I'd like you to decide whether to use the following source in your {type}. You should only use sources which contain important information that you don't already know from other sources or common knowledge. You'll only get three sources. If the text preview is an error message, say 'no.' Do you want to use the following source?\nURL: {url}\nText Preview: {text_preview}\nRespond only with 'yes' or 'no'.",
    'summarize_source_prompt': "\n\nNow I'd like you to summarize a source which will be helpful in writing your {type}. Your summary should be concise. Only include information which is directly relevant for providing useful factual context, and don't include information that you already know. Consider using direct quotes from the source. Here's the source.\nURL: {url}\nText: {text}"
}
...
def format_sources(summaries, urls):
    ...
    return prompts['displaying_sources_prompt'].format(sources=sources, type=type)


def gather_sources(prompt, type, endpoint, tweet, note=None, num_sources=3):
    """
    ...
    Here's the flow of how this function works:
        ...
        Return prompt
    """
    assert type in ["rating", "note"], "type must be Rating or Note"

    # Generate a search query
    prompt += prompts['search_query_prompt'].format(type=type)
        ...
        try:
            ...
            relevance_response = openai.chat.completions.create(
                ...
                messages=[{"role": "user", "content": prompt + format_sources(summaries, urls) + 
                    prompts['relevant_source_prompt'].format(url=url, text_preview=text_preview, type=type)
                }]
            ).choices[0].message.content
            ...
            if "yes" in relevance_response.lower():
                ...
                summary = openai.chat.completions.create(
                    model=endpoint,
                    messages=[{"role": "user", "content": prompt + format_sources(summaries, urls) + 
                        prompts['summarize_source_prompt'].format(url=url, text=truncated_text, type=type)
                    }]
                ).choices[0].message.content
